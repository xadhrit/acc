...
/* Structs or unions equal or smaller than 16 bytes are passed using up to two registers
if the first 8 bytes contains only floating-point type member they are passed in an xmm register. Otherwise , they are passed in a general purpose register

if a struct/union is larger than 8 bytes, the same rule is applied to the next 8 byte chunk

This function returns true if `ty` has only floating point members in its byte range[low, high]
*/ 

static bool has_floatnum(Type *ty, int low, int high, int offset){
   if (ty->kind == TY_STRUCT || ty->kind== TY_UNION){
      for (Member *mem = ty->members; mem; mem = mem->next){
         if (!has_floatnum(mem->ty, low, high, offset + mem->offset)){
            return false;
         }
      }
      return true;
   ...
   if (ty->kind == TY_ARRAY){
       for (int i=0; i < ty->array_len; i++){
          if (!has_floatnum(ty->base, low, high, offset + ty->base->size * i )){
             return false;
          }
       }
       return true;
   ...
   return offset < low || high <= offset || ty->kind == TY_FLOAT || TY_DOUBLE;
}

static bool has_floatnum1(Type *ty){
   return has_floatnum(ty, 0, 8, 0);
}

static bool has_floatnum2(Type *ty){
   return has_floatnum(ty, 8, 16, 0);
}
...
basically load arguments in registers or push them to the stack as specified by the x86-64 psABI. Here is what the spec says:

-Up to 6 arguments of integeral type are passed using RDI, RSI, RDX, RCX, R8 and R9
-UP to 8 arguments of floating-point type are passed using XMM0to XMM7
-Each argument passed on the stack takes 8 bytes, and  the end of the argument area must be aligned to a 16 bytes boundary.
- If a function is variadic, set the number of floating-point type arguments to RAX
*/

static int push_args(Node *node){
   int stack = 0, gp=0, fp=0;

   /* if the return type is a large struct/union , the caller a pointer a buffer as if it were the first argument
   ...
   */
   ...
   // if the return type is a large struct/union the caller to a pointer to a buffer as if it were the first argument
   ...
   if (has_floatnum1(ty)){
      ...
      if (ty->size == 4){
         ...
         for (int i =0; i < MIN(8, ty->size); i++){
            println(" mov %%al, %d(%%rbp)", var->offset + 1);
            println(" shr $8, %%rax ");
         }
         gp++;
      }
...
static void bultin_alloca(void){
   // Align size to 16 bytes
   println(" add $15 , %%rdi ");
   println(" and $0xffffff0 , %%edi");
...
// generate code a given node
static void gen_expr(Node *node){
   println(" .loc %d %d " , node->tok->file->file_no, node->tok->len);
   switch(node->kind){
      case ND_NULL_EXPR:
           return;
      case ND_NUM: {
           switch(node->ty->kind){
              ...
              case TY_LDOUBLE: {
                 ...
                 memset(&u,0, sizeof(u));
                 u.f80= node->fval;
                 println(" mov $%lu, %%rax #long double  %Lf", u.u64[0]);
      ...
      case ND_ASSIGN:
           ...
           if (node->lhs->kind == ND_MEMBER && node->lhs->member->is_bitfield){
              ...
              //if the left hand side of node is a bitfield, we need to read current value
     ...
     case ND_COND: {
      ...
      case ND_FUNCALL : {
           if (node->lhs->kind == ND_VAR && !strcmp(node->rhs->var->name, "alloca")){
              gen_expr(node->args);
              println("  mov  %%rax, %%rdi");
              builtin_alloca();
              return;
           ...
           //it looks like the most significant 48 or 56 bits in RAX may contain garbage if a function return type is short or bool/char,respectively.
           // We clear the upper bits here.
           switch(node->ty->kind){
           case TY_BOOL: 
                println(" movzx %%al , %%eax");
                return;
      ...
      case ND_CAS:{
           ...
           println(" lock cmpxchg %s ,(%%rdi)", reg_dex(sz));
           println(" sete %%cl ");
           println(" je 1f ");
      ...
      switch(node->lhs->ty->kind){
          ...
          case TY_LDOUBLE: {
             ...
             error_tok(node->tok, "invalid epxression");
          }
      }
      gen_expr(node->rhs); 
      push();
...
static void assign_lvar_offsets(Obj *prog){
   for (Obj *fn = prog;fn;fn=fn->next){
      ...
      /*
      if a function has many parameters,some parameters are inevitably passed by stack rather than by register.
      The first passed-by-stack parameter resides.
      */
      int top = 16;
      int bottom = 0;
...
static void emit_data(Obj *prog){
   for (Obj *var = prog; var; var = var->next){
     ...
     // .bss or .tbss
     if (var->is_tls){
           println(" .section .tbss,\"awT\", @progbits");
     }
     else {
        println(" .bss ");
     }
...
static void emit_text(Obj *prog){
   for (Obj *fn =prog; fn; fn = fn->next){
      ...
      println(" mov %%rsp, %d(%%rsp)", fn->alloca_bottom->offset );

      //save arg registers if function is variadic
      if (fn->va_area){
         int gp = 0, fp =0;
         for (Obj *var = fn->params; var; var = var->next){
             if (is_floatnum(var->ty)){
                fp++;
             }
             else {
                gp++;
...
void codegenarator(Obj *prog, FILE *out){
   out_file = out;
